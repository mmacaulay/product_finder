...
class LLMPrompt(models.Model):
    ...
    name = models.CharField(
        max_length=255, unique=True, help_text="Unique identifier for this prompt"
    )
    ...
    schema_version = models.CharField(
        max_length=20, default="1.0", help_text="Version of the response schema"
    )
    is_active = models.BooleanField(
        default=True, help_text="Whether this prompt is currently in use"
    )
    created_at = models.DateTimeField(auto_now_add=True)
    updated_at = models.DateTimeField(auto_now=True)
    ...
    def render(self, product):
        ...
        return self.prompt_template.format(
            ...
            additional_data=(
                str(product.de_product_data) if product.de_product_data else ""
            ),
        )
...
class LLMQueryResult(models.Model):
    ...
    product = models.ForeignKey(
        Product, on_delete=models.CASCADE, related_name="llm_results"
    )
    prompt = models.ForeignKey(
        LLMPrompt, on_delete=models.CASCADE, related_name="results"
    )
