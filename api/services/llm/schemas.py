...
class ResponseSchema:
    ...
    def get_json_template(self) -> str:
        ...
        for name, defn in self.fields.items():
            if defn.field_type is str:
                example = f'"{defn.description or "value"}"'
            elif defn.field_type is float:
                example = "0.85"
            elif defn.field_type is int:
                example = "1"
            elif defn.field_type is bool:
                example = "true"
            elif defn.field_type is list:
                example = '["item1", "item2"]'
            elif defn.field_type is dict:
                example = '{"key": "value"}'
            else:
                example = '"value"'
...
SAFETY_ANALYSIS_SCHEMA = ResponseSchema(
    ...
    fields={
        ...
        "summary": FieldDefinition(
            field_type=str, required=True, description="Brief safety overview"
        ),
...
def validate_response(data: dict, schema: ResponseSchema) -> Dict[str, Any]:
    ...
    for field_name, field_def in schema.fields.items():
        if field_name in data:
            ...
            # Type coercion
            try:
                if field_def.field_type is float and isinstance(value, (int, str)):
                    value = float(value)
                elif field_def.field_type is int and isinstance(value, (float, str)):
                    value = int(value)
                elif field_def.field_type is str and not isinstance(value, str):
                    value = str(value)
                elif field_def.field_type is bool and isinstance(value, str):
                    value = value.lower() in ("true", "yes", "1")
                elif field_def.field_type is list and not isinstance(value, list):
